...
if ($conn->connect_error){
	die("connection failed." . $conn->connect_error);
     ...
     $INSERT = "INSERT INTO log(log_lib_id, name, departmentORcurriculum, time_in, time_out) values(?, ?, ?, ?, ?)";

     $stmt = $conn->query($SELECT);
     if ($stmt->num_rows == 0) {
     	$stmt = $conn->prepare($INSERT);
     	$stmt->bind_param("sssii", $log_lib_id, $name, $departmentORcurriculum, $time_in, $time_out);
     	$stmt->execute();
...
<?php
    ...
    if ($result-> num_rows > 0){
        while ($row = $result-> fetch_assoc()){
            echo "<tr><td>". $row["log_lib_id"] . "</td><td>". $row["name"] . "</td><td>". $row["departmentORcurriculum"] . "</td><td>". $row["time_in"] . "</td><td>". $row["time_out"] . "</td></tr>";
        }
        echo "</table>";
    }
    else {
